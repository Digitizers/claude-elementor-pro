...
omits `cloudways-mcp` entirely, and doesn't capture the recent `claude-elementor-pro`
...
|---|---|---|---|
| **claude-elementor-pro** | `Digitizers/claude-elementor-pro` | Skill kit to build Elementor sites via the MCP — Pro/free + classic/V4 detection, brand-kit token intake, 10 section recipes, `new-client.sh` headless onboarding, Tier-0 ACF/JetEngine | building/onboarding a client Elementor site |
...
| **elementor-mcp (fork)** | `Digitizers/elementor-mcp` | The MCP **plugin** that exposes Elementor to the agent — Digitizers fork with the V4/atomic detection + save + styling fixes (the engine `claude-elementor-pro` drives) | installed on the site; the build layer underneath elementor-pro |
...
Status column: claude-elementor-pro / wordpress-api-pro / elementor-mcp = active &
...
| Onboard | claude-elementor-pro `new-client.sh` (wire the MCP + auth) + cloudways-mcp onboarding/audit |
| Brand setup | claude-elementor-pro brand-kit intake (`references/brand-kit.md`) |
| Build | claude-elementor-pro + recipes (`references/recipes.md`); elementor-mcp fork is the engine |
...
- `studio-toolbox.md` names all 5 tools (claude-elementor-pro, wordpress-api-pro,
